...
class CobbleHillsBookingBot {
    ...
    async handleLogin() {
        try {
            const loginForm = await this.driver.findElement(By.css('form[action*="login"]')).catch(() => null);
            if (loginForm) {
    ...
    async selectNextWednesday() {
        try {
            ...
            const datePicker = await this.driver.findElement(
                By.css('input[type="date"], input[name*="date"], .datepicker input')
            ).catch(() => null);
    ...
    async applyFilters() {
        try {
            ...
            // Click Cobble Hills Men's League 2025
            const leagueButton = await this.driver.findElement(
                By.xpath("//button[contains(text(), 'Cobble Hills Men\\'s League 2025')] | //div[contains(text(), 'Cobble Hills Men\\'s League 2025')]")
            );
            ...
            const playersSelect = await this.driver.findElement(
                By.css('select[name*="players"], select[id*="players"], input[name*="players"]')
            );
            
            const tagName = await playersSelect.getTagName();
            if (tagName === 'select') {
            ...
            const holesSelect = await this.driver.findElement(
                By.css('select[name*="holes"], select[id*="holes"], input[name*="holes"]')
            ).catch(() => null);
            
            if (holesSelect) {
                const tagName = await holesSelect.getTagName();
                if (tagName === 'select') {
                    await holesSelect.sendKeys('9');
                } else {
                    await holesSelect.clear();
                    await holesSelect.sendKeys('9');
                }
            } else {
                // Try clicking on 9 holes radio button
                const nineHolesOption = await this.driver.findElement(
                    By.xpath("//input[@type='radio' and @value='9'] | //button[contains(text(), '9')]")
                ).catch(() => null);
                if (nineHolesOption) {
            ...
            // Select "Any" cart option
            try {
                const cartSelect = await this.driver.findElement(
                    By.css('select[name*="cart"], select[id*="cart"]')
                );
                await cartSelect.sendKeys('Any');
            } catch (error) {
                const anyCartOption = await this.driver.findElement(
                    By.xpath("//input[@type='radio' and contains(@value, 'any')] | //button[contains(text(), 'Any')]")
                ).catch(() => null);
                if (anyCartOption) {
    ...
    async selectPreferredTimeSlot() {
        try {
            ...
            const timeSlots = await this.driver.findElements(
                By.css('.time-slot, .tee-time, [data-time], button[data-time]')
            );
            ...
            if (bestMatch && bestMatch.priority > 0) {
                ...
                const selectButton = await this.driver.findElement(
                    By.xpath("//button[contains(text(), 'Select')] | //input[@value='Select']")
                );
